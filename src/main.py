...
game_logics.draw_button(button_check,"Check")

button_change_three = pygame.Rect(235,830,150,60)
game_logics.draw_button(button_change_three,"Change 3")

button_hint = pygame.Rect(65,910,150,60)
game_logics.draw_button_hint(button_hint,hint)

button_newgame = pygame.Rect(235,910,150,60)
game_logics.draw_button(button_newgame,"New Game")
...
cartas = game_logics.tablero()
game_logics.eliminar_seleccionadas(config.list_tablero)

game_logics.write_points (text_points_window)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:  # Se activa cuando se presiona un botón del ratón
            ...
            if button_check.collidepoint(event.pos):
                ...
                if (len(selected)==3) :
                    if (game_logics.check(config.list_tablero[selected[0]],
                        ...
                        game_logics.draw_output_text(text_output_window,"SET CORRECTO!",G)
                        points = points + 1
                        game_logics.write_points(text_points_window)
                        if hint :
                        ...
                        if (new == "NULL"):
                            cartas = game_logics.change_three(selected,('NULL','NULL','NULL'))
                            game_logics.draw_output_text(text_output_window,"NO QUEDAN CARTAS!",R)
                        else:
                            game_logics.eliminar_seleccionadas(new)
                            cartas = game_logics.change_tablero(new)
                        
                    else : game_logics.draw_output_text(text_output_window,"ESO NO ES UN SET!",R)
                else : game_logics.draw_output_text(text_output_window,"LOS SETS SON DE 3 CARTAS!",R)
            ...
            if button_newgame.collidepoint(event.pos):
                ...
                points = 0
                game_logics.write_points(text_points_window)
                game_logics.draw_output_text(text_output_window,"PARTIDA NUEVA",(0,0,0))
            ...
            if button_change_three.collidepoint(event.pos):
                ...
                if (h == "NULL"):
                    game_logics.draw_output_text(text_output_window,"NO HABÍA NINGUN SET!",R)
                else : 
                    points = points - 1
                    game_logics.write_points(text_points_window)
                    game_logics.draw_output_text(text_output_window,"-1 punto por cambiar cartas",R)
                ...
                if (new_three == "NULL"):
                    ...
                    cartas = game_logics.change_three(old_three,new_three)
